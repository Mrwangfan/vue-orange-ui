/*
 * @Author: fan wang
 * @Date: 2022-01-26 09:54:05
 * @LastEditTime: 2022-01-26 10:15:54
 * @LastEditors: fan wang
 * @Description: 组件描述
 * @FilePath: \vue-orange-ui\build\build.js
 */
const fs = require('fs');
...
function build ({ input, output } = {}, index, arr) {
  ...
  run(
    `vue-cli-service lint && vue-cli-service build --target lib --no-clean  --name ${output} --dest ${getAssetsPath()} ${input}`
  );
